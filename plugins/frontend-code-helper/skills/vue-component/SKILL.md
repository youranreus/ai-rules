...
`components` 中应该递归地去编写子组件，每个子组件的文件结构与 `MyComponent` 相同。
...
1. `defineOptions` 定义组件选项，必须包含 `name` 属性，值为组件名称。
...
2. 遵守 DRY 原则，优先查看项目中的 `utils.ts` 文件，确认是否已经存在重复工具函数。
...
1. 对于样式，你需要判断项目是否使用 SCSS、是否使用 Tailwind。在 `style` 标签中，根据项目情况优先使用 `@apply` Tailwind 类名，其次使用 SCSS 类名。
